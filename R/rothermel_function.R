...
rothermel = function(surfFuel, moisture, crownFuel, enviro, rosMult = 1, cfbForm = "f", folMoist = "y") {
  ...
  #  ROS.final = ifelse(cfbForm == "sr" | cfbForm == "w", ifelse(type == "active" | type ==
   #     "passive", ros.surf + cfb.final * (ros.crown - ros.surf) * rosMult, ros.surf),
    ...
    fireBehavior =  data.frame(type,round(data.frame( cfb.final * 100, ROS.final, hpua.final * 11.35653,
        FI, FL, theta, ScorchHt/3.28084, TI, CI, SI, u.eff * 1.61, res.time),2))
    ...
    detailSurface = data.frame(ros.surf, nwns.ROS, sf, wf, mf.dead, mf.live*100,
    ...
    output = list(fireBehavior = fireBehavior, detailSurface = round(detailSurface,2),
